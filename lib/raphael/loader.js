...
var result = function (/*modules*/) {
    ...
    if (args.last && args.last.from)
        dir = args.last.from;


    if (result.debug) console.log("* loading raphael …");
    checkAndReadFile(dir + '/raphael/raphael.js', function (err, rcode) {
        if (err)
        ...
        if (args.length)
            result.G.apply(null, args.all.concat([end]));
        else end();
    });
};
...
result.G = function (modules) {
    ...
    var readModule = function (err, new_code) {
        ...
        if (module) {
            ...
            checkAndReadFile(dir+'/g.raphael/g.'+module+'.js',readModule);
        }
        else
