...
result.raphaelG = function (modules) {
    ...
    var readModule = function (err, new_code) {
        ...
        if (module) {
            ...
            fs.checkAndReadFile(depdir+'/g.raphael/g.'+module+'.js',readModule);
        }
        else
...
result.raphael = function (/*modules*/) {
    var args = new Vargs(arguments);
    if (result.debug) console.log("* loading raphael …");
    fs.chechAndReadFile(depdir + '/raphael/raphael.js', function (err, rcode) {
        if (err)
            return args.callback(err);
